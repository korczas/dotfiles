...
- **claude/**: Claude Code settings, hooks, and skills (symlinked into `~/.claude`)
...
#### Claude Code Config
`claude/settings.json`, `claude/hooks/`, and `claude/skills/` are symlinked to
`~/.claude/settings.json`, `~/.claude/hooks`, and `~/.claude/skills` by
`setup_symlinks.sh`, so Claude Code config is version-controlled here. The hook scripts
load secrets via `python-dotenv` at runtime (nothing sensitive is committed). Skills in
`claude/skills/` are personal user-level skills available in every project.
